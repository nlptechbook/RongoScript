...
Возможно, вам потребуется передать классификатору только придаточное предложение:

>сервер работает в нормальном режиме

При этом следует учитывать и сказуемое 'полагаю'. Другими словами необходимо выполнить более сложную, «умную» обработку ввода. RongoScript позволяет вам воспользоваться этой функциональностьюю. Для это вам нужно передать дополнительные параметры в метод get_response() модели. Пример такого вызова можно найти в разделе «Использование RongoScript в коде» позже.
